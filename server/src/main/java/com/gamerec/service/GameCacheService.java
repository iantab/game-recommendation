...
public class GameCacheService {
  ...
  public GameCacheService(
      IgdbClientService igdbClient,
      CachedGameRepository gameRepo,
      GenreRepository genreRepo,
      ThemeRepository themeRepo,
      PlatformRepository platformRepo,
      GameModeRepository gameModeRepo,
      PlayerPerspectiveRepository perspectiveRepo,
  ...
  public List<CachedGame> fetchAndCacheByGenresAndThemes(
      List<Integer> genreIds, List<Integer> themeIds, int limit) {
  ...
  /**
   * Ensures liked games are cached and fetches their similar games. Returns the list of similar
   * games as CachedGame objects.
   */
  ...
  public List<CachedGame> ensureCached(List<Long> ids) {
    ...
    List<Long> uncached =
        ids.stream().filter(id -> gameRepo.findById(id).map(this::isExpired).orElse(true)).toList();
    if (!uncached.isEmpty()) {
  ...
  public void loadReferenceData() {
    ...
    if (platformRepo.count() == 0) {
      ...
      platforms.forEach(
          p -> platformRepo.save(new Platform(p.id(), p.name(), p.slug(), p.category())));
    }
    ...
    if (perspectiveRepo.count() == 0) {
      ...
      perspectives.forEach(
          p -> perspectiveRepo.save(new PlayerPerspective(p.id(), p.name(), p.slug())));
    }
  }
  ...
  private void cacheGame(IgdbGame game) {
    try {
      ...
      cached.setSimilarGameIds(
          game.similarGames() != null ? game.similarGames().toArray(Long[]::new) : new Long[0]);
  ...
  private Long[] extractFranchiseIds(IgdbGame game) {
    ...
    return game.franchises().stream().map(f -> (long) f.id()).toArray(Long[]::new);
  }
  ...
  private GameDto toDto(IgdbGame game) {
    return new GameDto(
        ...
        game.cover() != null && game.cover().imageId() != null
            ...
            : null,
        game.genres() != null
            ? game.genres().stream().map(g -> new GenreDto(g.id(), g.name(), g.slug())).toList()
            : List.of(),
        game.themes() != null
            ? game.themes().stream().map(t -> new ThemeDto(t.id(), t.name(), t.slug())).toList()
            : List.of(),
        game.platforms() != null
            ? game.platforms().stream()
                .map(p -> new PlatformDto(p.id(), p.name(), p.slug(), p.category()))
                .toList()
            : List.of(),
        null);
  }
