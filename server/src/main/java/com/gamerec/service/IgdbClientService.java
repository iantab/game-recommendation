...
public class IgdbClientService {
  ...
  public IgdbClientService(
      IgdbConfig config, IgdbTokenManager tokenManager, ObjectMapper objectMapper) {
    this.config = config;
  ...
  public List<IgdbGame> searchGames(String query, int limit) {
    String body =
        GAME_FIELDS + " search \"" + sanitize(query) + "\"; limit " + clampLimit(limit) + ";";
  ...
  public List<IgdbGame> getGamesByGenres(List<Integer> genreIds, int limit) {
    ...
    String body =
        GAME_FIELDS
            + " where genres = ("
            + idList
            + ") & rating > 60 & rating_count > 5;"
            + " sort rating desc; limit "
            + clampLimit(limit)
            + ";";
    return queryGames(body);
  }

  public List<IgdbGame> getGamesByGenresAndThemes(
      List<Integer> genreIds, List<Integer> themeIds, int limit) {
    if ((genreIds == null || genreIds.isEmpty()) && (themeIds == null || themeIds.isEmpty()))
      return List.of();
    StringBuilder where = new StringBuilder();
    if (genreIds != null && !genreIds.isEmpty()) {
      where
          .append("genres = (")
          .append(genreIds.stream().map(String::valueOf).collect(Collectors.joining(",")))
          .append(")");
    }
    if (themeIds != null && !themeIds.isEmpty()) {
      ...
      where
          .append("themes = (")
          .append(themeIds.stream().map(String::valueOf).collect(Collectors.joining(",")))
          .append(")");
    }
    String body =
        GAME_FIELDS
