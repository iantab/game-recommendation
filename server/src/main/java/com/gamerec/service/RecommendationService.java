...
public class RecommendationService {
    ...
    public List<GameDto> getRecommendations(PreferenceRequest request) {
        ...
        List<ScoredGame> scored = candidatePool.stream()
    ...
    private PreferenceProfile buildProfile(PreferenceRequest request, List<Long> likedGameIds) {
        ...
        if (keywordWeights.size() > 20) {
            Map<Integer, Double> top20 = keywordWeights.entrySet().stream()
        ...
        return new PreferenceProfile(
                genreWeights, themeWeights, keywordWeights,
                gameModeIds, perspectiveIds, platformIds,
                developerSlugs, franchiseIds, similarGameIds, likedIds
        );
    }

    private void addWeights(Map<Integer, Double> weights, Integer[] ids, int totalGames, double implicitMultiplier) {
        if (ids == null) return;
    ...
    private Collection<CachedGame> buildCandidatePool(PreferenceRequest request, PreferenceProfile profile,
        ...
        if (!topGenres.isEmpty() || !topThemes.isEmpty()) {
            List<CachedGame> attrGames = cacheService.fetchAndCacheByGenresAndThemes(topGenres, topThemes, 50);
    ...
    private double scoreGame(CachedGame game, PreferenceProfile profile) {
        ...
        long highWeightGenreOverlap = gameGenres.stream()
                .filter(g -> profile.genreWeights.getOrDefault(g, 0.0) > 0.5)
                .count();
